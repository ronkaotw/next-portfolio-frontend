...
export default function AboutsText() {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.6, ease: "easeOut" }} 
     className="lg:w-full h-screen flex flex-col md:flex-row items-center justify-center gap-10 mt-3">
        ...
        <div className="col">
            <p className="relative w-md mt-5 sm:w-sm">{AboutText.text}</p>
        </div>
    </motion.section>
  );
}
